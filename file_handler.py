...
class FileHandler:
    ...
    def _process_single_asset(asset_data):
        category, item, textures, final_target, ped_name, base_path = asset_data
        ...
        try:
            ...
            if texture_dir.exists():
                ...
                for texture in textures:  # Only process textures from the selected list
                    ...
                    if texture_path.exists():
                        texture_temp = Path(final_target) / f"{temp_prefix}{texture_file}"
                        texture_final = Path(final_target) / f"{ped_name}^{texture_file}"
            ...
            item_int = int(item)  # Convert item to integer for formatting
            if category in ["watches", "glasses", "hats"]:
                model_file = f"{ped_name}_p^{category_prefix}_{item_int:03d}.ydd"  # Example: ig_test*_p*^p_head_001.ydd
                model_path = model_dir / f"{category_prefix}_{item_int:03d}.ydd"  # Original model file path
            else:
                model_file = f"{ped_name}^{category_prefix}_{item_int:03d}.ydd"  # Example: ig_test^accs_diff_003.ydd
                model_path = model_dir / f"{category_prefix}_{item_int:03d}_u.ydd"  # Original model file path
    ...
    def _copy_meta_files(ped_name):
        ...
        for meta_file, target_dir in meta_files.items():
            src = os.path.join(os.path.dirname(__file__), meta_file)
